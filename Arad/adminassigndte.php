...
ini_set('error_log', 'C:\\xampp\\htdocs\\sercon\\logs\\app_error.log');
...
class PanelScheduler
{
    ...
    private const MAX_PANELS_THURSDAY = 8;
    private const MAX_PANELS_OTHER_WEEKDAY = 12;
    private const MAX_PANELS_THURSDAY_AUTO = 8; // For this new method
    private const MAX_PANELS_OTHER_WEEKDAY_AUTO = 12; // For this new method
    ...
    private function getAllEligibleUnassignedPanelsSorted(): array
    {
        $progressConditions = [];
        foreach ($this->progressCols as $col) {
            $progressConditions[] = "`" . $col . "` IS NULL";
        }
        $progressSql = implode(" AND ", $progressConditions);

        $sql = "SELECT id, address, formwork_type, Proritization
                FROM hpc_panels
                WHERE plan_checked = 1
                  AND assigned_date IS NULL
                  AND (status IS NULL OR status NOT IN ('completed', 'cancelled'))
                  AND ({$progressSql})
                ORDER BY Proritization ASC, id ASC"; // Higher priority first
        $stmt = $this->pdo->query($sql);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    private function getInitialDailyUsage(string $minDate, string $maxDate): array
    {
        $dailyPanelCounts = [];
        $dailyFormworkUsage = []; // [date][formwork_base_type] => count

        // Initialize for all dates in range
        $currentDt = new DateTime($minDate);
        $endDt = new DateTime($maxDate);
        while ($currentDt <= $endDt) {
            $dateStr = $currentDt->format('Y-m-d');
            if (!$this->isFriday($currentDt)) { // Only care about workdays for initial counts
                $dailyPanelCounts[$dateStr] = 0;
                foreach (array_keys($this->dailyProcessCapacities) as $fwBaseType) {
                    $dailyFormworkUsage[$dateStr][$fwBaseType] = 0;
                }
            }
            $currentDt->modify('+1 day');
        }

        // Fetch existing scheduled panels in the date range
        $stmt = $this->pdo->prepare(
            "SELECT id, assigned_date, formwork_type
             FROM hpc_panels
             WHERE assigned_date BETWEEN :startDate AND :endDate"
        );
        $stmt->execute([':startDate' => $minDate, ':endDate' => $maxDate]);
        $existingAssignments = $stmt->fetchAll(PDO::FETCH_ASSOC);

        foreach ($existingAssignments as $assignment) {
            $assignedDateStr = $assignment['assigned_date'];
            if (isset($dailyPanelCounts[$assignedDateStr])) { // If it's a valid workday we are tracking
                $dailyPanelCounts[$assignedDateStr]++;
                $baseFw = $this->getBaseFormworkTypeExternal($assignment['formwork_type']);
                if ($baseFw && isset($dailyFormworkUsage[$assignedDateStr][$baseFw])) {
                    $dailyFormworkUsage[$assignedDateStr][$baseFw]++;
                }
            }
        }
        return [$dailyPanelCounts, $dailyFormworkUsage];
    }
    private function getFormworkInventory(): array // Renamed from getAvailableFormworkInstances for clarity
    {
        $inventory = [];
        $stmt = $this->pdo->query("SELECT formwork_type, available_count FROM available_formworks"); // Get all, even if 0
        foreach ($stmt->fetchAll(PDO::FETCH_KEY_PAIR) as $type => $count) {
            $baseType = $this->getBaseFormworkTypeExternal($type);
            if ($baseType) {
                $inventory[$baseType] = ($inventory[$baseType] ?? 0) + (int)$count; // Sum up if base types are duplicated with suffixes
            }
        }
        error_log("Formwork Inventory from DB: " . print_r($inventory, true));
        return $inventory;
    }
    ...
    public function newAutoScheduler(string $startDateStr, string $endDateStr, ?int $currentUserId): array
    {
        $successfullyScheduledPanels = []; // Panels successfully assigned in this run
        $logMessages = []; // For detailed step-by-step logging for one panel
        $allCandidatePanels = $this->getAllEligibleUnassignedPanelsSorted();

        if (empty($allCandidatePanels)) {
            return ['success' => true, 'message' => 'No eligible unassigned panels found to schedule.', 'scheduled' => [], 'unscheduled_candidates' => []];
        }
        error_log("newAutoScheduler: Found " . count($allCandidatePanels) . " total candidate panels.");

        // Date range (excluding Fridays)
        $operationalDateRange = [];
        $currentDt = new DateTime($startDateStr);
        $endDt = new DateTime($endDateStr);
        while ($currentDt <= $endDt) {
            if (!$this->isFriday($currentDt)) {
                $operationalDateRange[] = $currentDt->format('Y-m-d');
            }
            $currentDt->modify('+1 day');
        }
        if (empty($operationalDateRange)) {
            return ['success' => true, 'message' => 'No valid working days in the selected range.', 'scheduled' => [], 'unscheduled_candidates' => $allCandidatePanels];
        }
        error_log("Operational Date Range: " . implode(", ", $operationalDateRange));

        // Initial state of the world
        list($simulatedDailyPanelCounts, $simulatedDailyFormworkUsage) = $this->getInitialDailyUsage($operationalDateRange[0], end($operationalDateRange));
        $formworkStock = $this->getFormworkInventory(); // Total physical stock of each formwork type

        error_log("Initial Simulated Daily Panel Counts: " . print_r($simulatedDailyPanelCounts, true));
        error_log("Initial Simulated Daily Formwork Usage: " . print_r($simulatedDailyFormworkUsage, true));


        $remainingCandidatePanels = $allCandidatePanels; // We'll pull from this list

        // Iterate through each valid day in the operational range
        foreach ($operationalDateRange as $currentSchedulingDateStr) {
            $logMessages[] = "--- Processing Date: $currentSchedulingDateStr ---";
            $dayOfWeek = (new DateTime($currentSchedulingDateStr))->format('N');
            $maxPanelsForThisDay = ($dayOfWeek == 4) ? self::MAX_PANELS_THURSDAY_AUTO : self::MAX_PANELS_OTHER_WEEKDAY_AUTO;
            $panelsScheduledThisDay = 0; // Counter for panels added *in this run* for this day

            // Try to fill this day
            while (($simulatedDailyPanelCounts[$currentSchedulingDateStr] ?? 0) < $maxPanelsForThisDay && !empty($remainingCandidatePanels)) {
                $candidateFoundForThisSlot = false;
                $candidatePanelIndex = -1; // To remove panel if scheduled

                // Iterate through remaining candidates to find one that fits
                foreach ($remainingCandidatePanels as $index => $panel) {
                    $panelId = $panel['id'];
                    $baseFwType = $this->getBaseFormworkTypeExternal($panel['formwork_type']);

                    if (!$baseFwType) {
                        $logMessages[] = "Panel ID {$panelId}: Invalid base formwork type. Skipping.";
                        // Optionally move to a separate "permanently failed" list
                        unset($remainingCandidatePanels[$index]); // Remove from consideration
                        continue;
                    }

                    $formworkTypeSpecificDailyCapacity = $this->dailyProcessCapacities[$baseFwType] ?? 0; // Max of this TYPE per day
                    $currentUsageOfFwTypeThisDay = $simulatedDailyFormworkUsage[$currentSchedulingDateStr][$baseFwType] ?? 0;
                    $stockOfThisFwType = $formworkStock[$baseFwType] ?? 0;

                    $logMessages[] = "Panel ID {$panelId} ({$baseFwType}) for {$currentSchedulingDateStr}: DayLimit({$maxPanelsForThisDay}), DayCountNow({$simulatedDailyPanelCounts[$currentSchedulingDateStr]}), FwTypeDayLimit({$formworkTypeSpecificDailyCapacity}), FwTypeDayUsage({$currentUsageOfFwTypeThisDay}), FwStock({$stockOfThisFwType})";

                    // Check formwork specific capacity
                    if ($currentUsageOfFwTypeThisDay >= $formworkTypeSpecificDailyCapacity) {
                        $logMessages[] = "Panel ID {$panelId}: Limit for FW type {$baseFwType} on {$currentSchedulingDateStr} reached ({$currentUsageOfFwTypeThisDay}/{$formworkTypeSpecificDailyCapacity}). Trying next panel or day.";
                        continue; // Try next panel for this day, or this panel will be tried on next day
                    }
                    // Check against physical stock
                    if ($currentUsageOfFwTypeThisDay >= $stockOfThisFwType) {
                        $logMessages[] = "Panel ID {$panelId}: Stock limit for FW type {$baseFwType} reached ({$currentUsageOfFwTypeThisDay}/{$stockOfThisFwType}). Trying next panel or day.";
                        continue; // Not enough physical items of this type
                    }

                    // If we reach here, this panel CAN be scheduled in this slot!
                    $successfullyScheduledPanels[] = [
                        'panel_id' => $panelId,
                        'address' => $panel['address'],
                        'assigned_date' => $currentSchedulingDateStr,
                        'Proritization' => $panel['Proritization']
                    ];
                    // Update simulated counts
                    $simulatedDailyPanelCounts[$currentSchedulingDateStr]++;
                    $simulatedDailyFormworkUsage[$currentSchedulingDateStr][$baseFwType]++;

                    $candidateFoundForThisSlot = true;
                    $candidatePanelIndex = $index;
                    $logMessages[] = "Panel ID {$panelId} SCHEDULED for {$currentSchedulingDateStr}. Day count now: {$simulatedDailyPanelCounts[$currentSchedulingDateStr]}. FW {$baseFwType} count now: {$simulatedDailyFormworkUsage[$currentSchedulingDateStr][$baseFwType]}";
                    break; // Found a panel for this slot, break from inner foreach (panels)
                } // End foreach $remainingCandidatePanels

                if ($candidateFoundForThisSlot && $candidatePanelIndex !== -1) {
                    array_splice($remainingCandidatePanels, $candidatePanelIndex, 1); // Remove scheduled panel
                } else {
                    // No more candidate panels fit this specific day (either due to formwork or all tried)
                    $logMessages[] = "No more suitable candidate panels found for {$currentSchedulingDateStr} in current iteration.";
                    break; // Move to the next day
                }
            } // End while day not full and candidates remain
        } // End foreach $operationalDateRange


        // Database Update
        if (!empty($successfullyScheduledPanels)) {
            $this->pdo->beginTransaction();
            try {
                $stmtUpdate = $this->pdo->prepare(
                    "UPDATE hpc_panels SET assigned_date = :new_date,
                                          planned_finish_date = :planned_finish,
                                          planned_finish_user_id = :user_id
                     WHERE id = :panel_id"
                );
                foreach ($successfullyScheduledPanels as $assign) {
                    $newAssignedDateObj = new DateTime($assign['assigned_date']);
                    $newPlannedFinishDate = (clone $newAssignedDateObj)->modify('+1 day')->format('Y-m-d');
                    if (!$stmtUpdate->execute([
                        ':new_date' => $assign['assigned_date'],
                        ':planned_finish' => $newPlannedFinishDate,
                        ':user_id' => $currentUserId,
                        ':panel_id' => $assign['panel_id']
                    ])) {
                        throw new PDOException("DB update failed for panel ID: " . $assign['panel_id']);
                    }
                }
                $this->pdo->commit();
                error_log("newAutoScheduler: Committed " . count($successfullyScheduledPanels) . " assignments.");
            } catch (PDOException | Exception $e) {
                if ($this->pdo->inTransaction()) $this->pdo->rollBack();
                error_log("TRANSACTION FAILED in newAutoScheduler: " . $e->getMessage());
                // All successfullyScheduledPanels are now considered unscheduled due to transaction failure
                $failedDueToTx = $successfullyScheduledPanels;
                $successfullyScheduledPanels = []; // Reset
                foreach ($failedDueToTx as $item) $remainingCandidatePanels[] = $item; // Add them back to remaining
                return ['success' => false, 'error' => 'Database update failed. Transaction rolled back.', 'scheduled' => [], 'unscheduled_candidates' => $allCandidatePanels, 'log' => $logMessages];
            }
        }

        $unscheduledOutput = [];
        foreach ($remainingCandidatePanels as $panel) {
            $unscheduledOutput[] = ['panel_id' => $panel['id'], 'address' => $panel['address'], 'reason' => 'Not scheduled (no suitable slot or lower priority).'];
        }

        return [
            'success' => true,
            'message' => 'Auto-scheduling complete.',
            'scheduled' => $successfullyScheduledPanels,
            'unscheduled_candidates' => $unscheduledOutput, // Panels that were candidates but not placed
            'log' => $logMessages // For debugging one panel
        ];
    }
        ...
        try {
            $stmt = $this->pdo->prepare(
                "SELECT formwork_type, assigned_date, COUNT(*) as usage_count
                 FROM hpc_panels
                 WHERE assigned_date BETWEEN :startDate AND :endDate
                   AND formwork_type IS NOT NULL AND assigned_date IS NOT NULL
                 GROUP BY formwork_type, assigned_date"
            );
            $stmt->execute([':startDate' => $startDate, ':endDate' => $endDate]);
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach ($results as $row) {
                // IMPORTANT: Ensure you are grouping/counting by BASE formwork type if that's how capacities are defined
                $baseType = $this->getBaseFormworkTypeExternal($row['formwork_type']);
                ...
                $usage[$baseType][$row['assigned_date']] = ($usage[$baseType][$row['assigned_date']] ?? 0) + (int)$row['usage_count'];
            }
        } catch (PDOException $e) {
            error_log("DB Error in getAssignmentsCountByBaseTypeAndDate for PanelScheduler: " . $e->getMessage());
        }
        return $usage;
    }
    ...
    /**
     * Fetches ALL unassigned, plan_checked=1, non-progress, non-completed panels,
     * ordered by Proritization, then by ID.
     */
    private function getAllCandidatePanelsForAutoScheduling(): array
    {
        // Construct the dynamic part of the query for progress columns
        $progressConditions = [];
        foreach ($this->progressCols as $col) {
            $progressConditions[] = "`" . $col . "` IS NULL";
        }
        ...
        $sql = "SELECT id, address, formwork_type, Proritization, plan_checked
                FROM hpc_panels
                WHERE plan_checked = 1
                  AND assigned_date IS NULL
                  AND (status IS NULL OR status NOT IN ('completed', 'cancelled')) 
                  AND ({$progressSql}) /* Ensure no progress */
                ORDER BY Proritization ASC, id ASC"; // P1 first, then P2, etc.

        $stmt = $this->pdo->query($sql);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    /**
     * Auto-schedules a list of provided panels within a date range.
     * Used by the "Bulk Schedule" button.
     * Prioritizes based on Proritization, respects daily/Thursday limits, formwork capacities.
     * Overwrites existing assignments in the range if the panel has no progress.
     * Does NOT create polystyrene orders.
     */
    public function autoSchedulePanelsInRange(string $startDateStr, string $endDateStr, ?int $currentUserId): array
    {
        // Note: $panelsToAttempt from client is now IGNORED. This function gets all eligible panels.
        $successfullyScheduled = [];
        $couldNotBeScheduled = []; // For panels that are candidates but fail some check

        error_log("PHP autoSchedulePanelsInRange V2: ENTERED. Scheduling all eligible for $startDateStr to $endDateStr. UserID: " . ($currentUserId ?? 'NULL'));

        try {
            // 1. Prepare Date Range (excluding Fridays)
            $dateRange = [];
            $currentDt = new DateTime($startDateStr);
            $endDt = new DateTime($endDateStr);
            while ($currentDt <= $endDt) {
                if (!$this->isFriday($currentDt)) {
                    $dateRange[] = $currentDt->format('Y-m-d');
                }
                $currentDt->modify('+1 day');
            }

            if (empty($dateRange)) {
                return ['success' => true, 'message' => 'No valid working days in the selected range.', 'scheduled' => [], 'unscheduled' => []];
            }
            error_log("Valid date range for scheduling: " . implode(', ', $dateRange));

            // 2. Fetch ALL candidate panels from DB
            $allCandidatePanelsDetails = $this->getAllCandidatePanelsForAutoScheduling();
            if (empty($allCandidatePanelsDetails)) {
                return ['success' => true, 'message' => 'No eligible unassigned panels found in the database to schedule.', 'scheduled' => [], 'unscheduled' => []];
            }
            error_log("Found " . count($allCandidatePanelsDetails) . " total eligible candidate panels from DB.");

            // 3. Get current capacities and usage (these will be updated dynamically during the process)
            $formworkInventory = $this->getAvailableFormworkInstances();
            // Get usage for the entire potential range
            $formworkUsageByDate = $this->getAssignmentsCountByBaseTypeAndDate($dateRange[0], end($dateRange));
            $panelCountsByDate = [];
            foreach ($dateRange as $date) {
                $panelCountsByDate[$date] = $this->getScheduledPanelCountForDate($date);
                foreach (array_keys($this->dailyProcessCapacities) as $fwType) {
                    if (!isset($formworkUsageByDate[$fwType])) $formworkUsageByDate[$fwType] = [];
                    if (!isset($formworkUsageByDate[$fwType][$date])) $formworkUsageByDate[$fwType][$date] = 0;
                }
            }
            error_log("Initial formworkInventory: " . print_r($formworkInventory, true));
            error_log("Initial formworkUsageByDate: " . print_r($formworkUsageByDate, true));
            error_log("Initial panelCountsByDate: " . print_r($panelCountsByDate, true));


            $assignmentsToMakeDb = []; // Store {panel_id, new_date, address} for DB update

            foreach ($allCandidatePanelsDetails as $panel) {
                $panelId = $panel['id'];
                $panelAddress = $panel['address'];
                $logPrefix = "Attempting Panel (ID: {$panelId} - {$panelAddress}, Prio: {$panel['Proritization']}): ";

                // Plan_checked and progress already handled by getAllCandidatePanelsForAutoScheduling query

                $baseFwType = $this->getBaseFormworkTypeExternal($panel['formwork_type']);
                if (!$baseFwType) {
                    $couldNotBeScheduled[] = ['panel_id' => $panelId, 'address' => $panelAddress, 'reason' => 'Invalid formwork type in DB.'];
                    error_log($logPrefix . "Skipped - Invalid formwork type.");
                    continue;
                }

                $formworkTypeSpecificDailyLimit = $this->dailyProcessCapacities[$baseFwType] ?? 0;
                $totalInstancesOfThisTypeInStock = $formworkInventory[$baseFwType] ?? 0;

                if ($formworkTypeSpecificDailyLimit <= 0) {
                    $couldNotBeScheduled[] = ['panel_id' => $panelId, 'address' => $panelAddress, 'reason' => "No daily processing capacity for {$baseFwType}."];
                    error_log($logPrefix . "Skipped - No daily capacity for {$baseFwType}.");
                    continue;
                }
                if ($totalInstancesOfThisTypeInStock <= 0) {
                    $couldNotBeScheduled[] = ['panel_id' => $panelId, 'address' => $panelAddress, 'reason' => "No stock for {$baseFwType}."];
                    error_log($logPrefix . "Skipped - No stock for {$baseFwType}.");
                    continue;
                }

                $foundSlotForThisPanel = false;
                foreach ($dateRange as $dateStr) {
                    $dayOfWeek = (new DateTime($dateStr))->format('N');
                    $dailyPanelLimit = ($dayOfWeek == 4) ? self::MAX_PANELS_THURSDAY_AUTO : self::MAX_PANELS_OTHER_WEEKDAY_AUTO;
                    $currentPanelsOnThisDay = $panelCountsByDate[$dateStr] ?? 0; // Use current simulated count

                    // Check 1: Overall daily panel limit
                    if ($currentPanelsOnThisDay >= $dailyPanelLimit) {
                        // error_log($logPrefix . "Date {$dateStr} full (overall limit). Has {$currentPanelsOnThisDay}/{$dailyPanelLimit}");
                        continue;
                    }

                    $currentFwUsageOnDay = $formworkUsageByDate[$baseFwType][$dateStr] ?? 0; // Use current simulated count

                    // Check 2: Formwork type specific daily processing limit
                    if ($currentFwUsageOnDay >= $formworkTypeSpecificDailyLimit) {
                        // error_log($logPrefix . "Date {$dateStr} full for {$baseFwType} (daily process limit). Has {$currentFwUsageOnDay}/{$formworkTypeSpecificDailyLimit}");
                        continue;
                    }
                    // Check 3: Total physical instances of this formwork type in stock
                    if ($currentFwUsageOnDay >= $totalInstancesOfThisTypeInStock) {
                        // error_log($logPrefix . "Date {$dateStr} full for {$baseFwType} (stock limit). Has {$currentFwUsageOnDay}/{$totalInstancesOfThisTypeInStock}");
                        continue;
                    }

                    // Slot found for this panel on this date!
                    $assignmentsToMakeDb[] = [
                        'panel_id' => $panelId,
                        'new_date' => $dateStr,
                        'address' => $panelAddress // Keep for reporting
                    ];
                    // Increment usage counts FOR THE SIMULATION to affect subsequent panels/days
                    $panelCountsByDate[$dateStr]++;
                    $formworkUsageByDate[$baseFwType][$dateStr]++;
                    $foundSlotForThisPanel = true;
                    error_log($logPrefix . "SLOT FOUND on {$dateStr}. Day count now: {$panelCountsByDate[$dateStr]}. FW {$baseFwType} count now: {$formworkUsageByDate[$baseFwType][$dateStr]}");
                    break; // Move to the next panel
                } // End foreach $dateRange

                if (!$foundSlotForThisPanel) {
                    $couldNotBeScheduled[] = ['panel_id' => $panelId, 'address' => $panelAddress, 'reason' => "No suitable slot found in range for {$baseFwType} respecting all limits."];
                    error_log($logPrefix . "NO SLOT FOUND in entire range.");
                }
            } // End foreach $allCandidatePanelsDetails

            // 4. Perform Database Updates if any assignments were determined
            if (!empty($assignmentsToMakeDb)) {
                $this->pdo->beginTransaction();
                try {
                    $stmtUpdate = $this->pdo->prepare(
                        "UPDATE hpc_panels SET assigned_date = :new_date,
                    ...
                    foreach ($assignmentsToMakeDb as $assign) {
                        $newAssignedDateObj = new DateTime($assign['new_date']);
                        $newPlannedFinishDate = (clone $newAssignedDateObj)->modify('+1 day')->format('Y-m-d');
                        if (!$stmtUpdate->execute([
                            ':new_date' => $assign['new_date'],
                            ':planned_finish' => $newPlannedFinishDate,
                            ':user_id' => $currentUserId,
                            ':panel_id' => $assign['panel_id']
                        ])) {
                            // This specific assignment failed during DB update
                            error_log("DB update failed for panel ID during transaction: " . $assign['panel_id'] . " Error: " . print_r($stmtUpdate->errorInfo(), true));
                            // Add it to couldNotBeScheduled with a DB error reason, remove from successfullyScheduled if it was added optimistically
                            $couldNotBeScheduled[] = ['panel_id' => $assign['panel_id'], 'address' => $assign['address'], 'reason' => 'DB update failed within transaction.'];
                            // Continue to try other updates in the transaction? Or throw to rollback all?
                            // For atomicity, it's better to throw and rollback all.
                            throw new PDOException("DB update failed for panel ID: " . $assign['panel_id']);
                        }
                        $successfullyScheduled[] = $assign; // Add to final success list
                    }
                    $this->pdo->commit();
                    error_log("Successfully committed " . count($successfullyScheduled) . " panel assignments.");
                } catch (PDOException | Exception $e) {
                    if ($this->pdo->inTransaction()) $this->pdo->rollBack();
                    error_log("TRANSACTION FAILED in autoSchedulePanelsInRange: " . $e->getMessage());
                    // All $assignmentsToMakeDb are now considered failed
                    foreach ($assignmentsToMakeDb as $failedAssign) {
                        $isAlreadyListed = false;
                        foreach ($couldNotBeScheduled as $item) if ($item['panel_id'] == $failedAssign['panel_id']) $isAlreadyListed = true;
                        if (!$isAlreadyListed) $couldNotBeScheduled[] = ['panel_id' => $failedAssign['panel_id'], 'address' => $failedAssign['address'], 'reason' => 'DB Transaction Failed'];
                    }
                    $successfullyScheduled = []; // Reset successes because transaction failed
                    return ['success' => false, 'error' => 'Database update failed. Transaction rolled back.', 'scheduled' => [], 'unscheduled' => $couldNotBeScheduled];
                }
            }

            return [
                'success' => true,
                'message' => 'Auto-scheduling attempt complete.',
                'scheduled' => $successfullyScheduled,
                'unscheduled' => $couldNotBeScheduled
            ];
        } catch (PDOException $e) {
            error_log("PDO Error in autoSchedulePanelsInRange (outer): " . $e->getMessage());
            return ['success' => false, 'error' => 'Database error during preparation: ' . $e->getMessage(), 'scheduled' => [], 'unscheduled' => $couldNotBeScheduled];
        } catch (Exception $e) {
            error_log("General Error in autoSchedulePanelsInRange (outer): " . $e->getMessage());
            return ['success' => false, 'error' => 'An unexpected error occurred: ' . $e->getMessage(), 'scheduled' => [], 'unscheduled' => $couldNotBeScheduled];
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        elseif ($action === 'scheduleMultiplePanels') { // This is used by "Bulk Schedule" button
            if (!$canEdit) {
                $httpStatusCode = 403;
                $response = ['success' => false, 'error' => 'Permission denied.'];
                goto send_response; // Or your preferred way to exit
            }

            // The 'panels' POST data from the client is now IGNORED by the backend method
            // $panelsJson = $_POST['panels'] ?? '[]'; // We don't need this anymore for this action
            ...
            if (
                ...
                $httpStatusCode = 400;
                $response = ['success' => false, 'error' => 'Invalid date range provided.'];
                goto send_response; // Or your preferred way to exit
            }

            error_log("PHP AJAX Handler: 'scheduleMultiplePanels' (now newAutoScheduler) invoked. Start: $startDate, End: $endDate");

            $scheduler = new PanelScheduler($pdo, (int)$current_user_id, $formworkDailyCapacity, $progressColumns);

            // Call the new method signature: (startDate, endDate, userId)
            // The method itself will fetch all eligible panels from the DB.
            $result = $scheduler->newAutoScheduler($startDate, $endDate, (int)$current_user_id);

            error_log("PHP AJAX Handler: Result from newAutoScheduler: " . print_r($result, true));

            $httpStatusCode = $result['success'] ? 200 : 400;
            if (isset($result['error']) && (str_contains(strtolower($result['error']), 'database') || str_contains(strtolower($result['error']), 'db transaction'))) {
                $httpStatusCode = 500;
            }
            $response = $result;

            // No need for send_response_with_status: label here, will fall through to main one
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function clearCalendarSelection() {
            //console.log("--- clearCalendarSelection CALLED ---");
            //console.log("IDs to clear from selection visually:", Array.from(selectedCalendarEventIds));

            // Attempt direct visual deselection
            selectedCalendarEventIds.forEach(eventId => {
                const eventObj = calendar.getEventById(eventId.toString());
                if (eventObj && eventObj.el) {
                    toggleEventElementSelection(eventObj.el, false);
                } else {
                    console.warn(`clearCalendarSelection: Could not find .el for event ID ${eventId} to visually deselect.`);
                }
            });

            selectedCalendarEventIds.clear(); // Clear the state
            updateGlobalSelectionVisualState(); // Update body class

            // Force FullCalendar to re-evaluate classes for all visible events
            // This is crucial if direct .el manipulation above missed some or if FC overwrites it.
            //console.log("Forcing calendar event re-render after clear via setExtendedProp hack.");
            calendar.getEvents().forEach(event => {
                event.setExtendedProp('_clear_selection_trigger', Math.random());
            });
            // Alternatively, a heavier calendar.render() could be used if the above isn't enough.
            // setTimeout(() => calendar.render(), 0); // Use with caution
        }
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClassNames: function(arg) {
                // This is the primary source of truth for styling based on the Set
                if (selectedCalendarEventIds.has(arg.event.id.toString())) {
                    // console.log(`eventClassNames: ADDING selected class to ${arg.event.id}`);
                    return [CALENDAR_EVENT_SELECTED_CLASS];
                }
                // console.log(`eventClassNames: NOT ADDING selected class to ${arg.event.id}`);
                return [];
            },
            ...
            eventDrop: async function(info) {
                ...
                const physicallyDraggedEvent = info.event;
                const oldEventOriginalStart = moment(info.oldEvent.start); // Original start of the physically dragged event
                const newEventFinalStart = moment(physicallyDraggedEvent.start);
                const dateDeltaDays = newEventFinalStart.diff(oldEventOriginalStart, 'days');

                //console.log(`--- eventDrop START ---`); // ... (initial logs)

                let eventsToProcess = [];
                let originalEventPositions = new Map(); // PanelID => Date object

                // STEP 1: Determine ALL events involved and store their TRUE original positions BEFORE any calculations
                const isMultiDrag = selectedCalendarEventIds.size > 0 && selectedCalendarEventIds.has(physicallyDraggedEvent.id.toString());
                let idsInvolvedInDrag = new Set();

                if (isMultiDrag) {
                    selectedCalendarEventIds.forEach(idStr => idsInvolvedInDrag.add(idStr));
                } else {
                    idsInvolvedInDrag.add(physicallyDraggedEvent.id.toString());
                    if (selectedCalendarEventIds.size > 0) { // Dragged an unselected item while others were selected
                        clearCalendarSelection(); // Clear others, this drag is for this item only
                    }
                }

                idsInvolvedInDrag.forEach(eventIdStr => {
                    const eventObj = calendar.getEventById(eventIdStr);
                    if (eventObj) {
                        // If this is the event that was physically dragged, its "original" is info.oldEvent.start
                        // For other selected events, their "original" is their current eventObj.start
                        let originalStartForThisEvent;
                        if (eventObj.id.toString() === physicallyDraggedEvent.id.toString()) {
                            originalStartForThisEvent = info.oldEvent.start ? new Date(info.oldEvent.start.valueOf()) : (eventObj.start ? new Date(eventObj.start.valueOf()) : null);
                        } else {
                            originalStartForThisEvent = eventObj.start ? new Date(eventObj.start.valueOf()) : null;
                        }
                        originalEventPositions.set(eventObj.extendedProps.panelId, originalStartForThisEvent);
                    }
                });
                /*  console.log("Original positions stored:", Array.from(originalEventPositions.entries()).map(([k, v]) => ({
                     id: k,
                     start: moment(v).format()
                 }))); */


                // STEP 2: Populate eventsToProcess using the stored original positions and the common delta
                if (isMultiDrag) {
                    //console.log(`MULTI-DRAG detected. ${idsInvolvedInDrag.size} events to process.`);
                    document.body.classList.add('multi-dragging-calendar-events');
                    idsInvolvedInDrag.forEach(eventIdStr => {
                        const eventObj = calendar.getEventById(eventIdStr);
                        if (eventObj) {
                            const panelId = eventObj.extendedProps.panelId;
                            const originalStartForThisCalc = originalEventPositions.get(panelId);

                            if (!originalStartForThisCalc) {
                                console.error(`CRITICAL: No original position found for panelId ${panelId} in multi-drag!`);
                                // Skip this event or handle error
                                return;
                            }

                            const newTargetDateForThisEvent = moment(originalStartForThisCalc).add(dateDeltaDays, 'days');

                            eventsToProcess.push({
                                panelId: panelId,
                                address: eventObj.title,
                                formworkType: eventObj.extendedProps.formworkType,
                                originalEventInstance: eventObj,
                                newTargetDateStr: newTargetDateForThisEvent.format('YYYY-MM-DD'),
                                originalStoredStart: originalStartForThisCalc
                            });
                        }
                    });
                } else { // Single Drag
                    const panelId = physicallyDraggedEvent.extendedProps.panelId;
                    const originalStartForThisCalc = originalEventPositions.get(panelId);
                    if (!originalStartForThisCalc) {
                        console.error(`CRITICAL: No original position found for single dragged panelId ${panelId}!`);
                        // Fallback or error
                    }
                    eventsToProcess.push({
                        panelId: panelId,
                        address: physicallyDraggedEvent.title,
                        formworkType: physicallyDraggedEvent.extendedProps.formworkType,
                        originalEventInstance: physicallyDraggedEvent,
                        newTargetDateStr: newEventFinalStart.format('YYYY-MM-DD'), // Use its actual new dropped date
                        originalStoredStart: originalStartForThisCalc
                    });
                }

                if (eventsToProcess.length === 0) {
                    alert("No valid events to process for the drop operation.");
                    hideGlobalLoading();
                    if (physicallyDraggedEvent === info.event) info.revert(); // Revert the primary dragged event if nothing to process
                    document.body.classList.remove('multi-dragging-calendar-events');
                    clearCalendarSelection(); // Ensure selection is cleared
                    return;
                }

                /* console.log("Events to process in this drop operation:", eventsToProcess.map(e => ({
                    panelId: e.panelId,
                    newDate: e.newTargetDateStr
                }))); */
                showGlobalLoading(`در حال جابجایی ${eventsToProcess.length} پنل...`);
                let resultsLog = []; // To store success/failure message for each panel
                let allServerUpdatesSucceeded = true;
                for (const eventData of eventsToProcess) {
                    ...
                    let successForThisPanelBackend = false;
                    let messageForThisPanel = "";

                    try {
                        const availResponse = await makeAjaxCall({
                            action: 'checkFormworkAvailability',
                            panelId: panelId, // Pass panelId to exclude itself if it was already on newTargetDateStr (unlikely for a move)
                            date: newTargetDateStr,
                            formworkType: formworkType
                        });

                        if (!availResponse.available) {
                            throw new Error(availResponse.error || `ظرفیت برای ${formworkType} در ${newTargetDateStr} تکمیل است.`);
                        }
                        ...
                        if (updateResponse.progress_exists) {
                            if (confirm(`پنل "${address}" دارای اطلاعات پیشرفت است. برای جابجایی، اطلاعات پیشرفت پاک خواهد شد. ادامه می‌دهید؟`)) {
                                ...
                                throw new Error('جابجایی توسط کاربر به دلیل وجود پیشرفت لغو شد.');
                            }
                        }

                        if (updateResponse.success) { // Assuming updateResponse is the final one after progress handling
                            successForThisPanelBackend = true;
                            messageForThisPanel = updateResponse.message || 'با موفقیت جابجا شد.';
                            //console.log(`Backend success for Panel ID ${panelId} to ${newTargetDateStr}`);
                            // FullCalendar usually moves the physicallyDraggedEvent.
                            // For other events in a multi-drag, if they didn't visually update, ensure they do.
                            if (originalEventInstance.start.toISOString().split('T')[0] !== newTargetDateStr) { // Check if it's not already there
                                originalEventInstance.setStart(newTargetDateStr, {
                                    maintainDuration: true
                                });
                                //console.log(`Visually moved event ${panelId} on calendar.`);
                            }
                        } else {
                            throw new Error(updateResponse.error || 'به‌روزرسانی تاریخ در سرور ناموفق بود.');
                        }
                    } catch (e) {
                        allServerUpdatesSucceeded = false; // Mark that at least one backend update failed
                        successForThisPanelBackend = false;
                        messageForThisPanel = e.message;
                        console.warn(`Failed to move Panel ID ${panelId} ("${address}"): ${e.message}`);

                        // Revert THIS specific event visually on the calendar
                        if (originalEventInstance && originalStoredStart) {
                            originalEventInstance.setStart(originalStoredStart, {
                                maintainDuration: true
                            });
                        } else if (originalEventInstance === physicallyDraggedEvent) {
                            info.revert(); // FC's revert for the primary dragged item
                        }
                    }
                    resultsLog.push({
                        panelId,
                        address,
                        success: successForThisPanelBackend,
                        reason: messageForThisPanel
                    });
                } // End for...of eventsToProcess


                hideGlobalLoading();
                document.body.classList.remove('multi-dragging-calendar-events');

                let summaryAlertMessage = "نتایج جابجایی:\n";
                resultsLog.forEach(res => {
                    summaryAlertMessage += `${res.success ? '✅' : '❌'} ${res.address} (ID ${res.panelId}): ${res.reason}\n`;
                });
                alert(summaryAlertMessage);

                clearCalendarSelection(); // Clear our JS selection state

                // Decision for refetch:
                if (allServerUpdatesSucceeded) {
                    //console.log("All server updates reported success. Visuals should be mostly correct. Refetching for final sync.");
                    calendar.refetchEvents(); // If all backend calls said success, client-side moves should hold until refetch.
                } else {
                    console.warn("Not all server updates succeeded. Refetching events to ensure consistency with DB.");
                    // If there were failures, a refetch is crucial to show the true state
                    // because some client-side reverts might not have been perfect, or some events stayed.
                    calendar.refetchEvents();
                }

                //console.log(`--- eventDrop END --- Processed ${eventsToProcess.length} events. Succeeded: ${overallSuccessCount}`);
            },
            ...
            eventClick: async function(info) {
                ...
                if (info.jsEvent.ctrlKey || info.jsEvent.metaKey) {
                    ...
                    const isCurrentlySelected = selectedCalendarEventIds.has(clickedEventIdStr);
                    const isOnlySelected = isCurrentlySelected && selectedCalendarEventIds.size === 1;
                    ...
                    if (isOnlySelected) {
                        ...
                        selectedCalendarEventIds.clear(); // Clear the tracking set
                        selectedCalendarEventIds.add(clickedEventIdStr); // Add only the clicked one
                        toggleEventElementSelection(clickedEventEl, true); // Visually select the clicked one
                    }
                }
                updateGlobalSelectionVisualState();
                if (info.el) { // Ensure the clicked element's style is immediately updated
                    toggleEventElementSelection(info.el, selectedCalendarEventIds.has(info.event.id.toString()));
                }
            },
        ...
        bulkScheduleBtn.addEventListener('click', async function() {
            ...
            // Convert to Western Arabic numerals first
            startDateInputVal = toWesternArabicNumerals(startDateInputVal); // e.g., "1404/02/27"
            endDateInputVal = toWesternArabicNumerals(endDateInputVal); // e.g., "1404/02/31"

            //console.log("Converted Start Date Input:", startDateInputVal);
            //console.log("Converted End Date Input:", endDateInputVal);
            ...
            try {
                // --- New Parsing Logic ---
                const startParts = startDateInputVal.split('/');
                const endParts = endDateInputVal.split('/');

                if (startParts.length !== 3 || endParts.length !== 3) {
                    throw new Error("فرمت تاریخ ورودی صحیح نیست. باید YYYY/MM/DD باشد.");
                }

                const startJYear = parseInt(startParts[0], 10);
                const startJMonth = parseInt(startParts[1], 10) - 1; // moment-jalaali month is 0-indexed
                const startJDate = parseInt(startParts[2], 10);

                const endJYear = parseInt(endParts[0], 10);
                const endJMonth = parseInt(endParts[1], 10) - 1; // moment-jalaali month is 0-indexed
                const endJDate = parseInt(endParts[2], 10);

                // Create moment objects using the array + format method (often more robust for Jalali)
                mStartDate = moment().jYear(startJYear).jMonth(startJMonth).jDate(startJDate).startOf('day');
                mEndDate = moment().jYear(endJYear).jMonth(endJMonth).jDate(endJDate).startOf('day');
                // Alternatively, and often preferred for jalaali construction:
                // mStartDate = moment().jYear(startJYear).jMonth(startJMonth).jDate(startJDate);
                // mEndDate = moment().jYear(endJYear).jMonth(endJMonth).jDate(endJDate);
                // Let's try the array input first as it's more direct for parsing-like intent.

                //console.log("Attempted mStartDate (isValid?):", mStartDate.isValid(), mStartDate.toString());
                //console.log("Attempted mEndDate (isValid?):", mEndDate.isValid(), mEndDate.toString());
                ...
                startDate = mStartDate.format('YYYY-MM-DD'); // Convert to Gregorian for backend
            ...
            const panelsToSchedulePayload = Array.from(externalEventsContainer.querySelectorAll('.panel-item:not(.hidden):not([data-panel-assigned="true"]):not(.read-only-item)'))
                ...
                .filter(p => p.id);
            //console.log("JS Bulk Schedule: panelsToSchedulePayload count:", panelsToSchedulePayload.length);
            if (panelsToSchedulePayload.length > 0) {
                //console.log("JS Bulk Schedule: First panel ID to schedule:", panelsToSchedulePayload[0].id);
            } else {
                alert('JS DEBUG: No panels collected by querySelectorAll. Check panel list and selectors.');
                hideButtonLoading(this);
                return; // Make sure this path is indeed what's causing the alert you see.
            }

            if (panelsToSchedulePayload.length === 0) {
                alert('هیچ پنل واجد شرایطی (نمایش داده شده و تخصیص نیافته) برای زمانبندی خودکار یافت نشد.');
                hideButtonLoading(this);
                return; // This is likely where it's exiting if panels aren't collected
            }
            ...
            try {
                //console.log("JS Bulk Schedule: Making AJAX call to 'scheduleMultiplePanels'"); // LOG BEFORE AJAX
                const result = await makeAjaxCall({
                    action: 'scheduleMultiplePanels', // This now calls autoSchedulePanelsInRange on backend
                ...
                if (result.success !== undefined) {
                    calendar.refetchEvents(); // Good to refresh
                    let scheduledCount = result.scheduled?.length || 0;

                    // Update panel list UI for successfully scheduled panels
                    result.scheduled?.forEach(a => {
                        const pEl = document.querySelector(`.panel-item[data-panel-id="${a.panel_id}"]`);
                        if (pEl) {
                            pEl.dataset.panelAssigned = 'true';
                    ...
                    let msg = `<p>${result.message || (result.success ? 'عملیات انجام شد.' : ('خطا: ' + (result.error || 'خطای ناشناخته')))}</p>`;
                    if (scheduledCount > 0) {
                        msg += `<span class="font-semibold text-green-700">${scheduledCount} پنل با موفقیت زمان‌بندی شد.</span><br>`;
                    }

                    let cls = result.success ? ((result.unscheduled && result.unscheduled.length > 0) ? 'border-yellow-300 bg-yellow-50' : 'border-green-300 bg-green-50') : 'border-red-300 bg-red-50';
                    let unscheduledCount = result.unscheduled?.length || 0;

                    if (unscheduledCount > 0) {
                        msg += `<span class="font-semibold text-red-700">${unscheduledCount} پنل زمان‌بندی نشد:</span><ul class="list-disc list-inside mt-1 text-xs">`;
                        result.unscheduled.forEach(i => {
                            const pid = i.panel_id || '?';
                            // The 'address' might not be directly in i if it comes from autoSchedulePanelsInRange 'unscheduled'
                            // You might need to find it from the original client-side list if you want to display address here
                            const pInfo = panelsToSchedulePayload.find(p => p.id == pid); // panelsToSchedulePayload is from the client
                            const adr = pInfo ? (document.querySelector(`.panel-item[data-panel-id="${pid}"] .font-semibold`)?.textContent || `ID: ${pid}`) : `ID: ${pid}`;
                            msg += `<li class="text-gray-700">${adr}: ${i.reason}</li>`;
                        });
                        msg += `</ul>`;
                    }
                    ...
                    throw new Error("پاسخ نامعتبر از سرور (autoSchedulePanelsInRange).");
                }
            } catch (error) {
