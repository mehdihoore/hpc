                    ...
                    if (result.success) {
                        ...
                        if (result.shifted && result.shifted.length > 0) {
                            ...
                            result.shifted.forEach(p => {
                                msg += `<li>${p.full_address_identifier || 'ID: '+p.panel_id} (از ${moment(p.old_date).format('jYYYY/jMM/jDD')} به ${moment(p.new_date).format('jYYYY/jMM/jDD')})</li>`;
                            });
                            msg += `</ul>`;
            ...
            eventDrop: async function(info) {
                ...
                if (selectedCalendarEventIds.size > 0 && selectedCalendarEventIds.has(droppedEvent.id)) {
                    ...
                    selectedCalendarEventIds.forEach(eventId => {
                        ...
                        if (eventObj) {
                            eventsToMove.push({
                                ...
                                full_address_identifier: eventObj.title, // Or eventObj.extendedProps.full_address_identifier
                    ...
                    eventsToMove.push({
                        ...
                        full_address_identifier: droppedEvent.title,
                ...
                results.forEach(res => {
                    if (res.success) {
                        successCount++;
                        summaryMessage += `✅ ${res.full_address_identifier}: ${res.message || 'موفق'}\n`;
                    } else {
                        summaryMessage += `❌ ${res.full_address_identifier}: ${res.error}\n`;
            ...
            drop: async function(info) {
                ...
                for (const panelInfo of panelsToProcessClientInfo) {
                    ...
                    try {
                        ...
                        console.error(`Error processing panel ${panelfull_address_identifier} (ID: ${panelId}) during drop:`, error);
                        resultsSummary.push({
                            full_address_identifier: panelfull_address_identifier,
                            success: false,
                ...
                resultsSummary.forEach(res => {
                    if (res.success) {
                        summaryMsgHtml += `<div class="text-green-700">✅ ${res.full_address_identifier}: ${res.message || 'موفق'}</div>`;
                    } else {
                        failedCount++;
                        summaryMsgHtml += `<div class="text-red-700">❌ ${res.full_address_identifier}: ${res.reason}</div>`;
                    }
                });
