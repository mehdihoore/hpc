...
function insertCSVData($data, $table, $pdo)
{ // Pass $pdo, not $conn
    $results = [
        'success' => 0,
        'errors' => 0,
    ...
    for ($i = 1; $i < count($data); $i++) { // Start from 1 to skip header
        ...
        try {
            if ($table === 'hpc_panels') {
                ...
                if (empty($address) || empty($type) || $area <= 0 || $width <= 0 || $length <= 0) {
                ...
                if (empty($left_panel) || empty($right_panel)) {
                    throw new Exception("Left and right panels cannot be empty.");
                }
                ...
                if (empty($type) || $width <= 0 || $max_length <= 0) {
                    throw new Exception("Invalid data.");
                }
