...
$consolidatedStepsConfig = [
    ...
    'do_mesh' => [
        'label' => '1- مش بندی',
        'status_required' => 'pending',     // Must be pending to start
...
try {
    ...
    // --- Fetch Panels ---
    $stmt = $pdo->prepare("
        SELECT p.id, p.full_address_identifier, p.type, p.assigned_date, p.status, p.user_id,
        ...
        ORDER BY p.full_address_identifier
    ");
    $stmt->execute([$date]);
        ...
        elseif (isset($_POST['action'], $_POST['panel_id']) && !isset($_POST['step_name']) && $_POST['action'] !== 'save_qc_note') {
            ...
            try {
                ...
                elseif (substr($action, 0, strlen('undone_')) === 'undone_') {
                    ...
                    if (isset($consolidatedStepsConfig[$originalAction])) {
                        ...
                        $response['message'] = "Undo logic not found.";
                    }
                }
                // --- Handle Cancel Panel ---
                elseif ($action === 'cancel_panel') {
                    ...
                    if ($panelSuccess) {
                        ...
                        throw new PDOException("Failed to cancel panel.");
                    }
                } else {
            ...
            try {
                ...
                if ($stepName === 'qc_panel_image_upload' && isset($_FILES['qc_files'])) // Check for 'qc_files' (plural suggested name from JS)
                {
                    ...
                    if (!is_dir($qcUploadDirPath)) {
                        if (!mkdir($qcUploadDirPath, 0775, true)) {
                            throw new Exception("Failed to create QC upload subdirectory: " . $qcUploadDirPath);
                        }
                    }
                    if (!is_writable($qcUploadDirPath)) {
                        throw new Exception("QC Upload subdirectory is not writable: " . $qcUploadDirPath);
                    }
                    ...
                    for ($i = 0; $i < $numFiles; $i++) {
                        ...
                        if (!in_array($fileType, $allowedTypes)) {
                            error_log("Invalid file type: " . $fileType . " for file " . $originalFilename);
                            continue;
                        }
                        $maxSize = 5 * 1024 * 1024; // 5 MB
                        if ($fileSize > $maxSize) {
                            error_log("File size exceeds limit for file " . $originalFilename);
                            continue;
                        }
                        // --- End Validation ---
...
function getStatusClass($status)
{
    ...
    switch ($status) {
        ...
        case 'Formwork':
            return 'bg-yellow-100 text-yellow-700'; // Original color
        case 'Concreting':
            return 'bg-yellow-200 text-yellow-800'; // Use a distinct yellow
        case 'Assembly':
            return 'bg-purple-100 text-purple-800'; // Use purple for Assembly/Face Coat
        case 'completed':
...
<head>
    ...
    <style>
        ...
        #panel-details-container .close-details {
            position: absolute;
            top: 0.5rem;
            left: 0.5rem;
            cursor: pointer;
        ...
        #panel-details-container .close-details:hover {
        ...
        .undone-button:hover:not(:disabled) {
            background-color: #ffeeba;
        }

        .action-button-group .step-button.enabled {
            background-color: #0d6efd;
            color: white;
            cursor: pointer;
        }

        .action-button-group .step-button.enabled:hover {
            background-color: #0b5ed7;
        }

        .action-button-group .step-button.completed {
            background-color: #198754;
            color: white;
            cursor: default;
        }

        .action-button-group .step-button.disabled {
            background-color: #e9ecef;
            color: #adb5bd;
            cursor: not-allowed;
        }

        .action-button-group .undone-button.disabled {
            background-color: #e9ecef;
            color: #adb5bd;
            cursor: not-allowed;
        }
        ...
        .qc-step-item .qc-actions button {
            font-size: 0.75rem;
            padding: 2px 6px;
            border-radius: 3px;
        }
        ...
        .qc-undone-btn:hover:not(:disabled) {
        ...
        .qc-reject-btn:hover:not(:disabled) {
            background-color: #c82333;
        }
        ...
        .qc-file-input-label:hover {
            background-color: #138496;
        }

        .hidden-qc-file-input {
            display: none;
        }

        .qc-attachment-display span {
            font-size: 0.8em;
            margin-right: 5px;
            color: #555;
        }
        ...
        .pika-lendar {
            ...
            /* Force Vazir */
            float: none;
            /* Override default float for better positioning */
            margin: 5px auto;
            /* Center if needed */
        }
        ...
        .qc-note-display strong {
            color: #0056b3;
        }

        /* Optional: Highlight user who added note */
    </style>
</head>
...
<body class="bg-gray-100">
    <div class="container mx-auto px-4 py-8">
        ...
        <!-- Container for Panel Detail Report (shown on same page via full_address_identifier link) -->
            ...
            <table class="min-w-full" id="panels-table">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($panels) && !isset($pageError)): ?>
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-center text-gray-500">هیچ پنلی برای این تاریخ یافت نشد.</td>
                        </tr>
                        <?php
                            ...
                            <tr data-panel-id="<?php echo $panelId; ?>" class="<?php echo $isCancelled ? 'opacity-60 bg-red-50' : ''; ?>">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <a href="#" class="text-blue-600 hover:text-blue-900 panel-detail-link" data-panel-id="<?php echo $panelId; ?>">
                                        <?php echo htmlspecialchars($panel['full_address_identifier']); ?>
                                    </a>
                                </td>

                                <!-- Column 2: نوع -->
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <?php echo translate_panel_data_to_persian('panel_position', $panel['type'] ?? '-'); ?>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap action-cell">
                                    ...
                                    <div class="main-actions flex flex-wrap gap-2 items-center mb-2">
                                        ...
                                        <!-- QC Toggle Button -->
                                        <button class="qc-button <?php echo ($isCancelled /* || !$canEditPHP */) ? 'disabled' : ''; // Don't disable toggle itself based on role, content inside will be disabled 
                                                                    ?>" data-panel-id="<?php echo $panelId; ?>" <?php echo ($isCancelled) ? 'disabled' : ''; ?>>
                                            کنترل کیفی
                                        </button>
                                        <!-- Cancel Button -->
                                        <button class="cancel-button <?php echo ($isCancelled || !$canEditPHP) ? 'disabled' : ''; // MUST check role 
                                                                        ?>" data-panel-id="<?php echo $panelId; ?>" data-action="cancel_panel" <?php echo ($isCancelled || !$canEditPHP) ? 'disabled' : ''; ?>>
                                            لغو پنل
                                        </button>
                                    </div>
    ...
    <div id="operationModal" class="modal">
        <div class="modal-content">
            ...
            <p id="modalMessage"></p>
            <div class="mt-4 flex justify-end"> <button onclick="closeOperationModal()" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">بستن</button> </div>
        </div>
    </div>
    ...
    <script>
        (function() {
            ...
            // --- Modal Functions ---
            function showOperationModal(title, message, isSuccess = true) {
                /* ... (same as before) ... */
                modalTitle.textContent = title;
                modalMessage.textContent = message;
                modalTitle.className = isSuccess ? 'text-lg font-bold mb-4 text-green-700' : 'text-lg font-bold mb-4 text-red-700';
            ...
            function hidePanelDetailsReport() {
            ...
            async function handleMainActionClick(button) {
                if (!canEdit) {
                    console.warn("Edit attempt denied by role.");
                    return;
            ...
            function handleReportLinkClick(e) {
                ...
                fetch(url)
                    ...
                    .then(html => {
                        if (detailsPanelId === panelId) {
                            panelDetailsContainer.innerHTML = html;
                            const closeBtn = document.createElement('button');
                            closeBtn.textContent = '×';
                            closeBtn.className = 'close-details';
                            closeBtn.title = 'Close Details';
                            closeBtn.onclick = hidePanelDetailsReport;
                            panelDetailsContainer.appendChild(closeBtn);
                        }
                    })
                    .catch(error => {
                        console.error('Fetch Report Error:', error);
                        if (detailsPanelId === panelId) {
                            panelDetailsContainer.innerHTML = `<p class="text-center p-4 text-red-600">Error loading report: ${error.message}</p><button class="close-details" onclick="document.getElementById('panel-details-container').style.display='none'">×</button>`;
                        }
                    });
            }
            ...
            function handleSaveNoteClick(button) {
                if (!canEdit) {
                    console.warn("Save note denied by role.");
                    return;
                } // Guard
