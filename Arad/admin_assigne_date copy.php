    ...
    try {
        ...
        elseif ($action === 'getAssignedPanels') {
            try {
                ...
                $stmt = $pdo->prepare("SELECT hp.id, hp.full_address_identifier, hp.assigned_date, hp.planned_finish_date, hp.formwork_type, hp.status, hp.type,hp.Proritization, hp.widt,hp.floor
                                       FROM hpc_panels hp
...
try {
    ...
    $typesForFilter = array_values(array_unique(array_filter(array_column($filterData, 'floor')))); // Filter out empty/null types
    sort($typesForFilter);
