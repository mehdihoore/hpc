...
// --- Get Permissions ---
$stmtRole = $pdo->prepare("SELECT role FROM hpc_common.users WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'upload_file') {
            ...
            if (!($userPermissions['upload_files'] ?? false)) {
                $response['error'] = "You do not have permission.";
            } elseif (!$panelId || !in_array($orderType, ['east', 'west'])) {
                $response['error'] = "Invalid panel/order.";
            } elseif (empty($_FILES['design_file']['name'])) {
                ...
                if (!is_dir($baseUploadDir) && !mkdir($baseUploadDir, 0755, true)) {
                    $response['error'] = "Error creating base dir!";
                } else {
                    $panelDir = $baseUploadDir . $panelId . '/';
                    if (!is_dir($panelDir) && !mkdir($panelDir, 0755, true)) {
                        $response['error'] = "Error creating panel dir!";
                    } else {
                        $uploadDir = $panelDir . $orderType . '/';
                        if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true)) {
                            $response['error'] = "Error creating order dir!";
                        } else {
        ...
        elseif ($action === 'delete_file') {
            ...
            if (!($userPermissions['delete_all'] ?? false)) {
                $response['error'] = "You do not have permission.";
            } else {
                ...
                if ($realFullPath && $allowedDir && strpos($realFullPath, $allowedDir) === 0 && file_exists($realFullPath)) {
                    if (is_writable($realFullPath) && unlink($realFullPath)) {
                        $response['success'] = true;
                        $response['message'] = 'فایل پاک شد.'; // Use message key
                        log_activity($userId, 'file_delete', "File: " . $filePath);
                    } else {
                        $response['error'] = 'فایل پاک نشد یا مجوز کافی نیست!';
                    }
                } else {
                    $response['error'] = 'فایل یافت نشد یا مسیر نامعتبر است';
                }
            }
            $responseSent = true;
        }
        ...
        elseif ($action === 'update_status') {
            ...
            if (!($userPermissions['update_status'] ?? false)) {
        ...
        elseif ($action === 'create_order') {
            ...
            if ($panelId && !empty($orderType) && in_array($orderType, ['east', 'west'])) {
                ...
                if ($stmtIns->execute([$panelId, $orderType, $orderNum])) {
                    ...
                    $pdo->rollBack();
                    $response['error'] = "ایجاد سفارش موفقیت آمیز نبود!";
                }
            } else {
                $response['error'] = "Invalid panel/order type";
            }
            $responseSent = true;
        } else {
...
try {
    ...
    if (!empty($statusFilter) && $statusFilter != 'all') {
        $conditions[] = "(po_east.status = ? OR po_west.status = ?)";
        $params[] = $statusFilter;
        $params[] = $statusFilter;
    }
...
<head>
    ...
    <style>
        ...
        /* Ensure li are block for wrapping */
        .file-list a {
    ...
    <div class="container">
        ...
        <?php if (empty($panels) && !$error): ?>
            <div class="no-results">
                <p>هیچ پنلی با فیلترهای انتخاب شده یافت نشد.</p><a href="<?= strtok($_SERVER["REQUEST_URI"], '?') ?>" class="btn btn-primary mt-2">پاک کردن همه فیلترها</a>
            </div> <?php elseif (!empty($panels)): ?>
                ...
                <div id="panel-<?= $panel['panel_id'] ?>" class="card mb-3 <?= (!empty($searchAddress) && stripos($panel['address'], $searchAddress) !== false) ? 'panel-highlight' : '' ?>">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <h6 class="card-title mb-0">پنل: <?= escapeHtml($panel['address']) ?> <span class="text-muted small">(ID: <?= $panelId // $panelId is now guaranteed to be defined 
                                                                                                                                        ?>)</span></h6>
                        ...
                        <div class="panel-sides row g-3">
                            ...
                            <div class="panel-side col-md-6">
                                <div class="side-header">
                                    <h6 class="mb-0 small"><?= escapeHtml($panel['left_panel']) ?> </h6>
                                </div>
                                <p class="text-muted">
                                ...
                                <div class="order-details-west" data-order-id="<?= $panel['west_order_id'] ?>">
                                    ...
                                    ?>
                                        ...
                                        <?php else: echo '<span class="text-muted small">سفارشی ثبت نشده</span>';
                                        endif; ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                            ...
                            <div class="panel-side col-md-6">
                                <div class="side-header">
                                    <h6 class="mb-0 small"><?= escapeHtml($panel['right_panel']) ?> </h6>
                                </div>
                                <p class="text-muted">
        ...
        function formatJalaliDateOrEmptyJS(gregorianDate) {
            ...
            if (typeof persianDate !== 'undefined' && gregorianDate) {
                try {
                    ...
                    console.error("JS Date Format Error:", e);
        ...
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        try {
                            ...
                            if (serverResponse && serverResponse.error) errorMsg += " - " + serverResponse.error;
                        } catch (e) {}
            ...
            $(document).on('submit', '.file-upload-form', function(e) {
                ...
                if ($fileInput[0].files.length === 0) {
                    displayMessage('لطفا فایلی را برای آپلود انتخاب کنید.', 'danger');
                    return;
                }

                $submitButton.prop('disabled', true).data('original-text', $submitButton.text()).text('در حال آپلود...');
                $progressBarContainer.show();
                $progressBar.width('0%').attr('aria-valuenow', 0).text('');
                ...
                xhr.upload.addEventListener('progress', function(event) {
                    if (event.lengthComputable) {
                        ...
                        $progressBar.width(percentComplete + '%').attr('aria-valuenow', percentComplete); /* Removed text % */
                    }
                }, false);
                xhr.addEventListener('load', function(event) {
                    $progressBarContainer.hide();
                    $submitButton.prop('disabled', false).text($submitButton.data('original-text') || 'آپلود');
                    if (xhr.status >= 200 && xhr.status < 300) {
                        try {
                            var response = JSON.parse(xhr.responseText);
                            if (response.success) {
                                var $ul = $fileListContainer.find('ul.file-list');
                                if ($ul.length === 0) {
                                    $fileListContainer.html('<h6 class="small mt-2">فایل‌ها:</h6><ul class="file-list small p-0 m-0" style="list-style: none;"></ul>');
                                    $ul = $fileListContainer.find('ul.file-list');
                                }
                                ...
                                $ul.append(listItem);
                                $fileInput.val('');
                                $messageContainer.html(`<p class="text-success small">${escapeHtml(response.message)}</p>`);
                                setTimeout(() => {
                                    $messageContainer.empty();
                                }, 5000);
                            } else {
                                displayMessage(response.error || 'خطا در آپلود فایل.', 'danger');
                            }
                        } catch (e) {
                            console.error("Error parsing server response:", e, xhr.responseText);
                            displayMessage('پاسخ غیرمنتظره از سرور دریافت شد.', 'danger');
                        }
                    } else {
                        console.error("Upload failed with status:", xhr.status, xhr.statusText);
                        displayMessage(`خطا در آپلود (${xhr.status}: ${xhr.statusText})`, 'danger');
                    }
                });
                xhr.addEventListener('error', function(event) {
                    $progressBarContainer.hide();
                    $submitButton.prop('disabled', false).text($submitButton.data('original-text') || 'آپلود');
                    displayMessage('خطای شبکه.', 'danger');
                    console.error("Network error:", event);
                });
                xhr.addEventListener('abort', function(event) {
                    $progressBarContainer.hide();
                    $submitButton.prop('disabled', false).text($submitButton.data('original-text') || 'آپلود');
                    displayMessage('آپلود لغو شد.', 'warning');
                });
                var url = '<?= $_SERVER["PHP_SELF"] ?>?<?= http_build_query($_GET) ?>';
            ...
            $(document).on('submit', '.file-delete-form', function(e) {
                ...
                performAjaxAction(form, 'delete_file', function(response) {
                    $listItem.remove();
                });
            });
            ...
            $(document).on('submit', '.status-update-form', function(e) {
                ...
                var $form = $(form);
                var orderType = $form.find('input[name="order_type"]').val();
                var $statusSection = $form.closest('.mb-2').find('.status-section-' + orderType);
                var $submitButton = $form.find('button[type="submit"]');
                $submitButton.prop('disabled', true).data('original-text', $submitButton.text()).text('...');
                performAjaxAction(form, 'update_status', function(response) {
                    $statusSection.find('.badge').text(response.new_status_text).removeClass(function(i, c) {
                        return (c.match(/(^|\s)bg-\S+/g) || []).join(' ');
                    }).addClass('bg-' + response.new_status_color);
