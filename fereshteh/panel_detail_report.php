...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        LEFT JOIN users checker ON hp.plan_checker_user_id = checker.id -- Join to get checker username
        WHERE hp.id = ?
    ");
    ...
    $server_path = $_SERVER['DOCUMENT_ROOT'] . "/svg_files/" . $filename;
    $panel['svg_url'] = file_exists($server_path) ? "/svg_files/" . $filename : "";
