...
try {
    ...
    // hp.* will include the 'plan_checked' column automatically.
    $stmt = $pdo->prepare("
        SELECT hp.*
        FROM hpc_panels hp
...
<body class="bg-gray-100">
    <div class="container mx-auto px-4 py-8">
        ...
        <!-- Filter Grid -->
        <!-- MODIFIED: Changed lg:grid-cols-4 to lg:grid-cols-5 to better fit filters -->
    ...
    <script>
        ...
        function displayResults(panels) {
            resultsContainer.innerHTML = panels.map(panel => {
                ...
                const statusColor = statusColorMap[statusKey] || 'text-gray-700';
                ...
                // NEW: Logic to display plan checked status
                const planCheckedText = panel.plan_checked == '1' ? 'تایید شده' : (panel.plan_checked == '0' ? 'تایید نشده' : '-');
        ...
        function filterPanels() {
            ...
            filteredPanels = allPanels.filter(panel => {
                ...
                const matchesPanelType = !selectedPanelType || (panel.type && panel.type === selectedPanelType);
                ...
                return matchesSearch && matchesPrio && matchesFormwork && matchesStatus && matchesPanelType && matchesPlanChecked &&
        ...
        planCheckedFilter.addEventListener('change', filterPanels); // NEW: Event listener for the new filter
