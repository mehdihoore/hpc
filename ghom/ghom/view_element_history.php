<?php
// /public_html/ghom/view_element_history.php (OPTIMIZED & CORRECTED PHP)
...
if (!$element_id) {
    die("Element ID is required.");
}

// 2. FETCH ALL DATA IN A MORE EFFICIENT WAY
$history = [];
$element_info = null;
$template_items = [];
...
try {
    ...
    // Get the basic element info first
    ...
    if ($element_info) {
        // Fetch all historical inspections for this part
        $stmt_inspections = $pdo->prepare(
            "SELECT i.*, CONCAT_WS(' ', u.first_name, u.last_name) as user_name
             FROM inspections i
        ...
        // Fetch the correct checklist template for this element type
        ...
        // Organize the data for the JavaScript
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function jdate(format, timestamp) {
                if (!timestamp) return '';
                // Uses the browser's built-in international date formatting.
            ...
            function buildInspectionForm(inspectionRecord = null) {
                ...
                formContainer.innerHTML = '<h2>در حال بارگذاری فرم...</h2>';

                try {
                    if (!templateItems || templateItems.length === 0) {
                        throw new Error(`قالب چک لیست برای "${elementInfo.element_type}" یافت نشد.`);
                    }

                    // --- DATA-DRIVEN FORM BUILDING ---
                    // Group template questions by their stage
                    ...
                    const formChecklistHTML = Object.keys(itemsByStage).map(stageName => {
                        ...
                        <div class="status-selector">
                            ...
                            <label><input type="radio" name="status_${item.item_id}" value="N/A" ${status === 'N/A' ? 'checked' : ''}> N/A</label>
                        </div>
                        <input type="text" class="checklist-input" data-item-id="${item.item_id}" value="${value || ''}">
                    </div>`;
                        }).join('');
                        return `<div class="form-stage"><h4 class="stage-title">${stageName}</h4>${stageItemsHTML}</div>`;
                    }).join('');
                ...
                <form id="inspection-form">
                    ...
                    <fieldset class="consultant-section">
                        ...
                        <div class="form-group"><label>تاریخ بازرسی:</label><input type="text" name="inspection_date" data-jdp value="${inspData.inspection_date ? jdate(new Date(inspData.inspection_date).getTime()/1000) : ''}"></div>
                        ...
                        <div class="form-group"><label>یادداشت مشاور:</label><textarea name="notes" rows="3">${inspData.notes || ''}</textarea></div>
                    </fieldset>
                    <fieldset class="contractor-section">
                        <legend>بخش پیمانکار</legend>
                        <div class="form-group"><label>وضعیت:</label><select name="contractor_status"><option value="Pending" ${inspData.contractor_status === 'Pending' ? 'selected' : ''}>در انتظار</option><option value="Ready for Inspection" ${inspData.contractor_status === 'Ready for Inspection' ? 'selected' : ''}>آماده بازرسی</option></select></div>
                        <div class="form-group"><label>تاریخ اعلام:</label><input type="text" name="contractor_date" data-jdp value="${inspData.contractor_date ? jdate(new Date(inspData.contractor_date).getTime()/1000) : ''}"></div>
                        <div class="form-group"><label>یادداشت پیمانکار:</label><textarea name="contractor_notes" rows="3">${inspData.contractor_notes || ''}</textarea></div>
                    </fieldset>
                    <hr>
                    <h3>چک لیست جزئیات</h3>
                    ${formChecklistHTML}
                    <div class="btn-container" style="display:flex; justify-content:space-between; align-items:center;">
                         <button type="button" id="new-inspection-btn" class="btn btn-new">ثبت به عنوان بازرسی جدید</button>
                         <button type="submit" class="btn btn-save">${isEditMode ? 'ذخیره تغییرات در این سابقه' : 'ثبت بازرسی جدید'}</button>
                    </div>
                </form>`;
            ...
            function setFormPermissions(formEl, role) {
                ...
                if (consultantSection) consultantSection.disabled = (role !== 'admin');
                if (contractorSection) contractorSection.disabled = (role !== 'supervisor');
            }
            ...
            async function handleFormSubmit(event) {
                ...
                try {
                    ...
                    saveBtn.textContent = editingInspectionId ? 'ذخیره تغییرات' : 'ثبت بازرسی جدید';
                }
            }
            ...
            // Initial load of a blank form when the page opens
