 ...
 */
function applyStatusColors(svgElement, dbData) {
  if (!svgElement) return;

  // First, set all interactive elements to their default "Pending" color
  svgElement.querySelectorAll(".interactive-element").forEach((el) => {
    ...
    if (elementType === "GFRC") {
      ...
      const colors = svgGroupConfig.GFRC?.colors;
      if (colors) {
        el.style.fill = orientation === "Horizontal" ? colors.h : colors.v;
      } else {
        el.style.fill = STATUS_COLORS.Pending;
      }
    } else {
      // For all other types, use the generic grey pending color.
      el.style.fill = STATUS_COLORS.Pending;
    }
  });

  // Second, override with specific statuses from the database.
  if (dbData) {
    for (const elementId in dbData) {
      const data = dbData[elementId];
      const el = svgElement.getElementById(elementId);

      if (el && data.status && data.status !== "Pending") {
...
function applyGroupStylesAndControls(svgElement) {
  ...
  if (!isPlan) {
    applyStatusColors(svgElement, currentPlanDbData);
  }
}
...
document.addEventListener("selectionChanged", (e) => {
  ...
  if (!allSameType) {
    batchStageSelect.innerHTML =
      '<option value="">المان‌های انتخابی باید از یک نوع باشند</option>';
    batchStageSelect.disabled = true;
    return;
  }
  ...
  batchStageSelect.innerHTML =
    '<option value="">در حال بارگذاری مراحل...</option>';
  ...
  fetch(`/ghom/api/get_stages.php?type=${firstElementType}`)
    ...
    .then((stages) => {
      if (stages && stages.length > 0) {
        ...
        batchStageSelect.innerHTML =
          '<option value="">مرحله‌ای برای این نوع المان تعریف نشده</option>';
        batchStageSelect.disabled = true;
      }
    });
});
