...
function openChecklistForm(fullElementId, elementType, dynamicContext) {
  ...
  formPopup.style.display = "flex"; // Use flex to activate the new CSS layout
  ...
  fetch(`/ghom/api/get_element_data.php?${apiParams.toString()}`)
    ...
    .then((data) => {
      ...
      let formBodyHTML =
        '<p class="no-stages-message">مراحل بازرسی برای این المان تعریف نشده است.</p>';
                      ...
                      <div class="stage-body">
                          ...
                          <div class="stage-sections">
                              <fieldset class="consultant-section">
                                  ...
                                  <div class="form-group"><label>وضعیت کلی:</label><select name="overall_status"><option value="Pending" ${
                                    !stageHistory.overall_status ||
                                    stageHistory.overall_status === "Pending"
                                      ? "selected"
                                      : ""
                                  }>در انتظار</option><option value="OK" ${
              stageHistory.overall_status === "OK" ? "selected" : ""
            }>تایید</option><option value="Not OK" ${
              stageHistory.overall_status === "Not OK" ? "selected" : ""
            }>رد</option></select></div>
                                  <div class="form-group"><label>تاریخ بازرسی:</label><input type="text" name="inspection_date" value="${
                                    stageHistory.inspection_date_jalali || ""
                                  }" data-jdp readonly></div>
                              </fieldset>
                              <fieldset class="contractor-section">
                                  <legend>بخش پیمانکار</legend>
                                  <div class="form-group"><label>وضعیت:</label><select name="contractor_status"><option value="Pending" ${
                                    !stageHistory.contractor_status ||
                                    stageHistory.contractor_status === "Pending"
                                      ? "selected"
                                      : ""
                                  }>در حال اجرا</option><option value="Ready for Inspection" ${
              stageHistory.contractor_status === "Ready for Inspection"
                ? "selected"
                : ""
            }>آماده</option></select></div>
                                  <div class="form-group"><label>تاریخ اعلام:</label><input type="text" name="contractor_date" value="${
    ...
    .catch((err) => {
      formContainer.innerHTML = `<div class="form-loader"><h3>خطا</h3><p>${escapeHtml(
        err.message
      )}</p><button type="button" class="btn cancel" onclick="closeForm('universalChecklistForm')">بستن</button></div>`;
    });
}
/**
...
function setFormState(formContainer, userRole, history) {
  ...
  formContainer.querySelectorAll(".stage-container").forEach((stageEl) => {
    ...
    // Find the latest history record for this specific stage.
    ...
    if (isSuperuser) {
      ...
      // Contractor can only edit if the consultant has NOT approved it yet,
      // and they haven't already marked it as 'Ready for Inspection'.
      if (
        ...
        if (rejectionCount < 3) {
          ...
          // If rejected 3 times, lock the section.
  ...
  if (saveButton) {
    if (isSuperuser || isConsultant || isContractor) {
