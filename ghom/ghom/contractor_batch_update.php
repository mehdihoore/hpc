...
<body data-user-role="<?php echo escapeHtml($_SESSION['role']); ?>">
    ...
    <div id="batch-update-panel">
        ...
        <div class="form-group">
            <label for="batch_status">تغییر وضعیت به:</label>
    ...
    <footer>
        <p>&copy; 1404-1405 شرکت آلومنیوم شیشه تهران.</p>
    </footer>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('selectionChanged', async (e) => {
                ...
                try {
                    ...
                    // 1. Fetch all possible stages for this element type
                    const stagesResponse = await fetch(`/ghom/api/get_stages.php?type=${firstElementType}`);
                    const allStages = await stagesResponse.json();
                    ...
                    // 2. Fetch the current status of the selected elements
                    const statusResponse = await fetch('/ghom/api/get_selection_status.php', {
                        method: 'POST',
                        headers: {
                    ...
                    const elementStatuses = await statusResponse.json();

                    // 3. Determine the next valid stage for the entire group
                    let maxLastPassedOrder = -1; // Start before the first stage (order 0)
                    elements.forEach(el => {
                        const lastPassed = elementStatuses[el.element_id];
                        if (lastPassed === undefined && maxLastPassedOrder < 0) {
                            // This element is new, its last passed stage is effectively -1
                            maxLastPassedOrder = Math.max(maxLastPassedOrder, -1);
                        } else if (lastPassed !== undefined) {
                            maxLastPassedOrder = Math.max(maxLastPassedOrder, parseInt(lastPassed, 10));
                        }
                    });

                    const nextStageOrder = maxLastPassedOrder + 1;

                    // 4. Filter the list of all stages to show only the next valid one(s)
                    // For now, we only show the very next stage.
                    ...
                    if (availableStages.length > 0) {
                        ...
                        batchStageSelect.innerHTML = '<option value="">مرحله بعدی یافت نشد (شاید تمام مراحل تمام شده؟)</option>';
                        batchStageSelect.disabled = true;
                    }
