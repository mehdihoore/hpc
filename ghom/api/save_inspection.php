...
{
    // Return null immediately if the date is empty or not a string.
    ...
    if (count($parts) !== 3 || $parts[0] < 1300) { // Basic validation
        return null;
    }
...
function createHistoryEntry(array $stageData, int $userId, string $userRole, ?array $newAttachments, ?array $newContractorAttachments): array
{
    $entry = [
        ...
        'user_id' => $userId,
        'role' => $userRole,
        'action' => '', // Will be set based on role
        'data' => []
    ];
    ...
    // Always include checklist items if they exist
...
try {
    ...
    foreach ($stagesData as $stageId => $stageData) {
        ...
        // --- MODIFIED: Process uploads once and get arrays for history log ---
        $newAttachmentsJson = processAndMergeUploads('attachments', $fullElementId, $existing_inspection['attachments'] ?? null);
        $newContractorAttachmentsJson = processAndMergeUploads('contractor_attachments', $fullElementId, $existing_inspection['contractor_attachments'] ?? null);

        // Get the array version for the history log
        ...
        if ($existing_inspection) {
            ...
            if ($userRole === 'admin' || $userRole === 'superuser') {
                ...
                $params['attachments'] = $newAttachmentsJson; // Use the merged JSON string
            }
            ...
            if (in_array($userRole, ['cat', 'car', 'coa', 'crs']) || $userRole === 'superuser') {
                ...
                $params['contractor_attachments'] = $newContractorAttachmentsJson; // Use the merged JSON string
            }
            ...
            // --- ADDED: Append to history log on UPDATE ---
            ...
            write_log("   Appended to history_log for inspection ID: $inspectionId");
            // --- END ADDITION ---

        } else {
            write_log("   Action: INSERT new record.");

            // --- ADDED: Create history entry before insert ---
            $historyEntry = createHistoryEntry($stageData, $userId, $userRole, $newAttachmentsArray, $newContractorAttachmentsArray);
            // --- END ADDITION ---
            ...
            $params = [
                ...
                'attachments' => $newAttachmentsJson, // Use the processed JSON string
                ...
                'contractor_attachments' => $newContractorAttachmentsJson, // Use the processed JSON string
                // --- ADDED: Include history_log on insert ---
                'history_log' => json_encode([$historyEntry], JSON_UNESCAPED_UNICODE)
            ];
